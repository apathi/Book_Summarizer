...
```
┌──────────┐    ┌─────────────┐    ┌──────────────┐    ┌─────────────┐
│ PDF File │───▶│ TOC Parser  │───▶│ Chapter      │───▶│ Page Range  │
│          │    │             │    │ Detector     │    │ Calculator  │
└──────────┘    └─────────────┘    └──────────────┘    └─────────────┘
                                                                 │
┌─────────────┐    ┌─────────────┐    ┌──────────────┐         │
│Final Report │◀───│ PDF         │◀───│ Directory    │◀────────┘
│& Cleanup    │    │ Extractor   │    │ Creator      │
└─────────────┘    └─────────────┘    └──────────────┘
```
...
```
┌───────────┐    ┌─────────────┐    ┌──────────────┐    ┌─────────────┐
│ EPUB File │───▶│ Direct      │───▶│ Chapter      │───▶│ Image       │
│           │    │ Extraction  │    │ Classification│    │ Extraction  │
└───────────┘    └─────────────┘    └──────────────┘    └─────────────┘
                                                                  │
┌─────────────┐    ┌─────────────┐    ┌──────────────┐         │
│Final Report │◀───│ PDF         │◀───│ HTML         │◀────────┘
│& Cleanup    │    │ Conversion  │    │ Creation     │
└─────────────┘    └─────────────┘    └──────────────┘
```
...
```
┌─────────────┐    ┌─────────────┐    ┌──────────────┐    ┌─────────────┐
│ PDF Chapter │───▶│ Text        │───▶│ Content      │───▶│ AI API      │
│             │    │ Extraction  │    │ Validation   │    │ Call        │
└─────────────┘    └─────────────┘    └──────────────┘    └─────────────┘
                                                                    │
┌─────────────┐    ┌─────────────┐                               │
│ Markdown    │◀───│ Summary     │◀──────────────────────────────┘
...
├── Uses: ChapterDetector  
│   ├── scan_for_chapters()
│   ├── merge_toc_with_pages()
...
└── Internal: PDF Extraction
    ├── create_chapter_pdfs()
    └── _create_pdf_section_directories()
```

### EPUB Processing Components  
```
EPUBProcessor
├── Uses: EPUBImageExtractor
│   └── extract_chapter_images()
├── Uses: HTMLToPDFConverter
│   ├── convert_html_to_pdf()
│   └── _cleanup_image_folder()
├── Uses: ReportGenerator  
│   ├── generate_processing_report()
│   └── print_processing_summary()
└── Internal: EPUB Extraction
    ├── _extract_epub_chapters_direct()
    ├── _extract_epub_title()
    └── _create_epub_chapter_html_with_images()
```
...
```
INITIALIZED → PARSING → DETECTING → EXTRACTING → REPORTING → COMPLETE
     │           │          │           │           │           │
     ▼           ▼          ▼           ▼           ▼           ▼
   Ready    TOC Found   Chapters    Files      Reports     Success
            │          Detected    Created    Generated      │
            ▼              │          │           │         ▼
         Failed            ▼          ▼           ▼      Cleanup
...
### API Key Management
```
.env file → python-dotenv → Environment Variables → API Clients
    │                                │
    ▼                                ▼
