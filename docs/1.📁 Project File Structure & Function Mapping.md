# 📁 Book Summarizer - Complete File Structure & Function Mapping
...
│
├── AI_summarizer/                          # 🤖 AI Summarization Tools
...
│   ├── pdf_processor.py                   # 📄 Complete PDF workflow processor
│   ├── epub_processor.py                  # 📚 Complete EPUB workflow processor
│   ├── toc_parser.py                      # 📋 Table of Contents parsing
│   ├── chapter_detector.py                # 🔍 Chapter detection algorithms
│   ├── epub_image_extractor.py            # 🖼️ EPUB image extraction & processing
...
│   └── *.epub                             # 📖 Source EPUB books
│
├── book_chapters/                          # 📁 Processed Output
│   ├── book-name_chapters/                # 📂 Individual book folders
│   │   ├── A._Section_Name/               # 📁 Section-based organization (PDFs)
│   │   │   ├── Chapter_01-Title.pdf      # 📄 Individual chapter PDFs
│   │   │   └── Chapter_02-Title.pdf
│   │   ├── Chapters/                      # 📁 Flat organization (EPUBs)  
│   │   │   ├── Chapter_01-Title.pdf      # 📄 EPUB chapters as PDFs (with images)
│   │   │   ├── Chapter_02-Title.pdf
│   │   │   ├── Extra_01-Introduction.pdf # 📄 Supporting content
│   │   │   └── *_chatgpt_summary.md      # 🤖 AI-generated summaries
│   │   │   └── *_claude_summary.md       # 🧠 AI-generated summaries
│   │   └── book-name_processing_report.json # 📊 Processing metadata
│   └── [additional books...]
│
└── test_chapters_clean_section/           # 🧪 Test Output Directory
    └── [same structure as book_chapters/]
```
...
### 🎯 **book_processing/main.py** (Ultra-Minimal Orchestrator - 45 lines)
```python
class BookProcessor:
...
### 📄 **book_processing/pdf_processor.py** (Complete PDF Workflow - 200 lines)
```python
class PDFProcessor:
...
**8-Step PDF Workflow**:
1. Parse Table of Contents → Uses TOCParser
2. Scan for actual chapter pages → Uses ChapterDetector  
3. Merge TOC with detected pages → Uses ChapterDetector
4. Calculate page ranges → Uses ChapterDetector
5. Show preview & get confirmation → Uses ReportGenerator
6. Create section directories → Internal logic
7. Extract individual PDFs → Uses PyPDF2
8. Generate reports → Uses ReportGenerator

---

### 📚 **book_processing/epub_processor.py** (Complete EPUB Workflow - 400 lines)
```python
class EPUBProcessor:
...
1. Extract EPUB structure → Direct document parsing
2. Show preview with smart numbering → Chapter vs Extra classification  
3. Create directories → Flat "Chapters" structure
4. Extract chapters → HTML creation → Image extraction → PDF conversion

---

### 📋 **book_processing/toc_parser.py** (Table of Contents Parsing)
```python
class TOCParser:
...
### 🔍 **book_processing/chapter_detector.py** (Chapter Detection Algorithms)
```python
...
### 🖼️ **book_processing/epub_image_extractor.py** (EPUB Image Processing)
```python
...
### 🔄 **book_processing/html_to_pdf_converter.py** (HTML→PDF Conversion)
```python
...
### 📊 **book_processing/report_generator.py** (Processing Reports)
```python
...
### 🛠️ **book_processing/utils.py** (Shared Utilities)
```python
...
### 💬 **AI_summarizer/chatgpt_summarizer.py** (OpenAI Integration)
```python
...
### 🧠 **AI_summarizer/claude_summarizer.py** (Anthropic Integration)
```python
...
### 📄 **AI_summarizer/pdf_text_extractor.py** (PDF Text Extraction)
```python
...
### 📝 **AI_summarizer/prompt_template.py** (AI Prompts)
```python
